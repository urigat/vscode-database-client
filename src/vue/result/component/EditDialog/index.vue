        ...
        <el-form ref="infoForm" :model="editModel" :inline="true">
            ...
            >
                <template>
                    <CellEditor
                        v-if="editModel"
                        v-model="editModel[column.name]"
                        :type="column.type"
                    ></CellEditor>
                </template>
                <template v-if="column.type === 'jsonb'">
                    <h3>Optional Primary Key</h3>
                    <span>
                        <el-input
                            :value="optionalPrimaryKey"
                            @input="setOptionalPrimaryKey"
                            placeholder="Optional Primary Key"
                        ></el-input>
                        <!-- :<el-input :value="optionalPrimaryKeyValue" @input="sync" placeholder="Value"></el-input> -->
                    </span>
                </template>
            </el-form-item>
        </el-form>
...
export default {
    ...
    methods: {
        setOptionalPrimaryKey(value) {
            this.optionalPrimaryKey = value;
            this.primaryKey = value;
            this.primaryKeyList = [{ name: value, simpleType: null }];
        },
        ...
        showPreview() {
            this.previewSQL =
                this.model == "update"
                    ? this.buildUpdateSql(this.editModel, this.originModel)
                    : this.buildInsertSQL();
            if (!this.previewSQL) {
    ...
    computed: {
        editorTilte() {
            if (this.model == "insert") {
                ...
                return `Edit For ${this.table} : ${
                    this.primaryKey
                }=${assertString(this.originModel[this.primaryKey])}`;
            } else {
